...
func main() {
	...
	flag.StringVar(&alignflag, "align", "left", "alignment of the ascii art")
	flag.Parse()
	...
	if ascii.IsFlagPassed("color") {
		if len(argsPassed) == 1 {
			ascii.Color(colorflag, lettersTocolor, argsPassed, bannerContent, outputflag, alignflag)
		...
		if len(argsPassed) == 2 {
			...
			argsPassed = argsPassed[:1]
			colorCode = ""
			ascii.Art(argsPassed, bannerContent, lettersTocolor, colorCode, 0, outputflag, alignflag)
